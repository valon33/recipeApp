...
const AppProvider = ({ children }) => {
    ...
    const getRecipes = async () => {
        ...
        try {
            ...
            const recipes = await httpRequest.getRecipes();
    ...
    // const getRecipes = async () => {
    //     const { isLoading, data, error } = useGetRecipes();
    //     isLoading && dispatch({ type: "LOADING" });

    //     data &&
    //         dispatch({ type: "GET_RECIPES", payload: data.data.data.recipe });

    //     error && dispatch({ type: "ERROR", payload: error.response });
    // };
